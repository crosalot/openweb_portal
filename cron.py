...
def create(site):
    ...
    # create_database
    DB_NAME = DB_PASS = DB_USER = SITE_NAME.replace('.', '_').replace('-', '_')[0:15]
    ...
    os.system('mysqldump -u%s -p%s %s < %s%s/db.sql' % (DB_USER, DB_PASS, DB_NAME, OW_PLATFORM, PACKAGE_CODE))
    ...
    # Avahi setiing 
    os.system('python %s %s.%s'  % (OW_AVAHI, SITE_NAME, OW_DOMAIN))
